...
export const EditPrivateLeaderboardWeights = () => {
    ...
    useEffect(
        () => {
            ...
            //     }
            // )
            let privateLeaderboardInfoDict = TokenService.getPrivateLeaderboardInfoDict() ?? {};
            if (params.privateLeaderboardUUID in privateLeaderboardInfoDict) {
                let privateLeaderboardInfo = privateLeaderboardInfoDict[params.privateLeaderboardUUID];
                setIsOwner(privateLeaderboardInfo.isOwner === "1");
                setPrivateLeaderboardName(privateLeaderboardInfo.name);
                PrivateLeaderboardService.getPrivateLeaderboardWeightings(
                    params.privateLeaderboardUUID
                ).then(
                    (res2) => {
