...
GRAALVM_VERSION=`native-image.cmd --version`
echo "[-->] Compiling Spring Boot App '$ARTIFACT' with $GRAALVM_VERSION"
time native-image.cmd -J-Xmx4G -H:+TraceClassInitialization -H:Name=$ARTIFACT -H:+ReportExceptionStackTraces -Dspring.native.remove-unused-autoconfig=true -Dspring.native.remove-yaml-support=true  -cp $CP $MAINCLASS;
# time native-image \
#   -J-Xmx4G \
